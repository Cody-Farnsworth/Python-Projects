...
def create_db(self):
    ...
    with conn:
        cur = conn.cursor()
        cur.execute("CREATE TABLE if not exists tbl_studentsinfo( \
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_studentsinfo (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','English'))
            conn.commit()
    conn.close()
...
def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_studentsinfo""")
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_studentsinfo WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_studentsinfo WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_studentsinfo (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_course))
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_studentsinfo WHERE col_fullname = '{}'""".format(var_select))
...
def onRefresh(self):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT(*) FROM tbl_studentsinfo""")
        count = cursor.fetchone()[0]
        i = 0
        while i < count:
                cursor.execute("""SELECT col_fullname FROM tbl_studentsinfo""")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_studentsinfo WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_studentsinfo WHERE col_email = '{}'""".format(var_email))
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_studentsinfo SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_value))
                        onClear(self)
                        conn.commit()
                else:
